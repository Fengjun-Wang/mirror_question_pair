...
class Siamese_LSTM(nn.Module):
    def __init__(self,
        ...
        self.linear1 = nn.Linear(2 * self.num_directions * hidden_size,
                                 self.linear_hid_size)  ##out dim of q1 is 2*hidden(concat two hiddens), same for q2
...
class Model(object):
    ...
    def save_plot_(self):  ##TODO add save function
        ...
        res[["train_acc", "val_acc", "train_loss", "val_loss"]].to_csv(
            self.name + "/%s_train_process.csv" % (self.name), index=False)
...
bidirectional = True
lstm_drop_p = 0.4
lstm_input_drop_p = 0.5
linear_hidden_size = 600  ##
linear_hid_drop_p = 0.2
batch_size = 256  ##
folder = 5
early_stop = 10
LR = 0.001
...
def train_main():
    ...
    val_dg = DataGenerator(data_df=xval_df, space=space, bucket_num=5, batch_size=512, is_prefix_pad=False,
                           is_shuffle=False, is_test=False)
    test_dg = DataGenerator(data_df=test_df, space=space, bucket_num=5, batch_size=512, is_prefix_pad=False,
                            is_shuffle=False, is_test=True)
...
def cv_main():
    ...
    test_df = DataSet.load_test()
    test_dg = DataGenerator(data_df=test_df, space=space, bucket_num=5, batch_size=256, is_prefix_pad=False,
                            is_shuffle=False, is_test=True)
