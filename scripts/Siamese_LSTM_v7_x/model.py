...
class Siamese_LSTM(nn.Module):
    def __init__(self,
                 ...
                 is_freeze,
                 lstm_hidden_size,
                 input_CNN_out_channel,
                 input_CNN_kernel_size,
                 number_layers,
                 lstm_dropout_p,
                 bidirectional,
                 linear_hid_size,
        ...
        self.input_CNN_out_channel = input_CNN_out_channel
        self.input_CNN_kernel_size = input_CNN_kernel_size
        self.input_CNN_padding = int(math.ceil(float(self.input_CNN_kernel_size-1)/2))
        ...
        self.input_conv = nn.Conv1d(  in_channels=self.input_channel_len,
                                      out_channels=self.input_CNN_out_channel,
                                      kernel_size=self.input_CNN_kernel_size,
                                      stride=1,
                                      padding=self.input_CNN_padding)
        self.batch_norm = nn.BatchNorm1d(input_CNN_out_channel)
        self.lstm = nn.LSTM(input_size=self.input_CNN_out_channel,
                            hidden_size=self.lstm_hidden_size,
                            num_layers=self.number_layers,
                            batch_first=True,
                            dropout=self.lstm_dropout_p,
                            bidirectional=self.bidirectional)
        self.linear1 = nn.Linear(2*self.num_directions*lstm_hidden_size,self.linear_hid_size) ##out dim of q1 is 2*hidden(concat two hiddens), same for q2
    ...
    def init_hidden(self,batch_size):
        self.hidden = ( torch.zeros(self.number_layers*self.num_directions, batch_size, self.lstm_hidden_size),
                        torch.zeros(self.number_layers*self.num_directions, batch_size, self.lstm_hidden_size))
...
space = "words"
is_freeze = False ##
input_CNN_out_channel = 100
input_CNN_kernel_size = 2
...
def train_main():
    ...
    siamese_lstm = Siamese_LSTM(pre_trained_embedding=item_embed,
                               ...
                               input_CNN_out_channel = input_CNN_out_channel,
                               input_CNN_kernel_size = input_CNN_kernel_size,
